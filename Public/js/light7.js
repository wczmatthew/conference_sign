...
+ function($) {
  ...
  //$(".page").before("<p style='position:fixed;z-index:999999;'>"+his+"</p>");
  ...
  $(document).on("click", "[href]", function(e) {
    ...
    // 2017-3-20 gaocw 添加开始
    // 增加 loading='none' 就不会有loading效果   
    if($(this).attr("loading") == "none" )
        return;
    // 2017-3-20 gaocw 添加结束
    
    //independent-m独立模块自带样式
    if($(this).attr("href").indexOf("javascript:history.go(-1)") >=0) {

        if(window.history.length <=1){
            $.myToast("无效后退，已是最顶层页面", 2000, 'warning');
            return;
        }
    }else{
        $.myLoading();
  ...
  "use strict";
    ...
    //modify byGS
     $.showIndicator = function () {
        ...
        var $loading = $('<div class="preloader-indicator-overlay"></div><div class="preloader-indicator-modal"><span class="preloader preloader-white"></span></div>').appendTo(defaults.modalContainer);

        $.openModal($loading);
    };    
    $.hideIndicator = function () {
        var $loading = $('.preloader-indicator-overlay, .preloader-indicator-modal').remove();
        $.closeModal($loading);
    };   
    // Action Sheet
    ...
    //添加byGS 
    $.myLoading = function(){
        var $loading = $('<div class="modal loading-chint"></div><div class="preloader-indicator-modal"><span class="preloader preloader-white"></span></div>').appendTo(document.body);
        $.openModal($loading);
    }
    //添加byGS 
    $.myToast = function (msg, duration, extraclass) {
        var $toast = $('<div class="modal toast ' + (extraclass || '') + '"><i class="icon-toast "></i><p class="toast-content">' + msg + '</p></div>').appendTo(document.body);
        $.openModal($toast);
         setTimeout(function() {
                $.closeModal($toast);
        }, duration || 2000);
    ...
    $.toast = function(msg, duration, extraclass) {
        var $toast = $('<div class="modal toast toast-time ' + (extraclass || '') + '"><i class="icon-toast "></i><p class="toast-content">' + msg + '</p></div>').appendTo(document.body);
        $.openModal($toast);
         setTimeout(function() {
                $.closeModal($toast);
        }, duration || 2000);
    ...
    /*$.myToast = function (msg, duration, extraclass) {
        var $toast = $('<div class="modal toast ' + (extraclass || '') + '"><i class="icon-toast "></i><p class="toast_content">' + msg + '</p></div>').appendTo(document.body);
        $.openModal($toast);
          setTimeout(function() {
            $.closeModal($toast);
          }, time || 2000);
    ...
    //修改byGS:添加在popup中显示picker和toast
    $.openModal = function (modal) {
        ...
        var isToast = modal.hasClass('toast-time');
        var isLoading = modal.hasClass('loading-chint');
        if(defaults.closePrevious) {
            if(isPopover || isPopup){
                $.closeModal();
            }
            if(isPickerModal){
                //$.closeModal();
                /*$(".picker-modal").each(function(index,v){
                    if(index!=0)
                        $.closeModal(v);
                });*/
                $.closeModal($(".picker-modal:not(:last)"));
                //$.closeModal($(".toast-time"));
            }
            if(isToast){
                $.closeModal($(".toast-time:not(:last)"));
            }
            //$.closeModal();
        }
        ...
        var overlay;
        if (!isLoginScreen && !isPickerModal && !isToast && !isLoading) {
        ...
        if (!isLoginScreen && !isPickerModal && !isToast && !isLoading) overlay.addClass('modal-overlay-visible');
      ...
      p.updateValue = function () {
          ...
          for (var i = 0; i < p.cols.length; i++) {
              if (!p.cols[i].divider) {
                // 2017-3-21 gaocw
                //                 
                if(typeof p.cols[i].value == "undefined"){
                    value = p.cols[i].values[0];
                }
                else{
                    value = p.cols[i].value;
                }
                  newValue.push(value);
                  newDisplayValue.push(value);
...
+ function($) {
  "use strict";
  $.fn.yearmonthPicker = function(params) {
    return this.each(function() {
      ...
      var initYears = (function () {
        ...
        if(today.getMonth() == 0)
            to_year = to_year-1;
        for (var i = 2005; i <= to_year; i++) { arr.push(i); }
        return arr;
      })();

/*  //若工资允许查询当月的，用以下：
      var year_selected = (function () {
        var year = today.getFullYear();
        return year;
      })();

      var month_selected = (function () {
...
*/
      ...
      var defaults = {
        ...
        value: [year_selected, formatNumber(month_selected)],

        onChange: function (picker, values, displayValues) {

          var currentMonthValue = picker.cols[2].value;
          var currentYearValue = picker.cols[0].value;


          if(currentMonthValue > month_selected && currentYearValue == year_selected ){
            currentMonthValue = month_selected;
            //console.log(currentMonthValue);
          }
          picker.cols[2].setValue(formatNumber(currentMonthValue));
          //var days = getDaysByMonthAndYear(picker.cols[1].value, picker.cols[0].value);//获取月份列表
          //var currentValue = picker.cols[2].value;//获取选中值
...
+ function($) {
  ...
  $.fn.datetimeYMDHPicker = function(params) {
    return this.each(function() {
      ...
      var defaults = {
        ...
        value: [today.getFullYear(), formatNumber(today.getMonth()+1), today.getDate(), formatNumber(today.getHours())],
        ...
        formatValue: function (p, values, displayValues) {
          return displayValues[0] + '-' + values[1] + '-' + values[2] + ' ' + values[3] + ':00';
        },
...
+ function($) {
  ...
  $.fn.datetimeYMPicker = function(params) {
    return this.each(function() {
      ...
      var initYears = (function () {
        var arr = [];
        for (var i = cur_year - 1; i <= cur_year + 2; i++) { arr.push(i); }
        return arr;
      })();
