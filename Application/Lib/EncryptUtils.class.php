...
class EncryptUtils
{
   ...
   */
	 static function AESEncrypt($plaintext,$old_key){
    # 密钥应该是随机的二进制数据，
    # 开始使用 scrypt, bcrypt 或 PBKDF2 将一个字符串转换成一个密钥
    # 密钥是 16 进制字符串格式
    //$key = pack('H*', $old_key);
    $key = HEXDecode($old_key);
    ...
    //var_dump(HEXEncode($ciphertext));
    # 将初始向量附加在密文之后，以供解密时使用
    //$ciphertext = $iv . $ciphertext;
    
    # 对密文进行 base64 编码
    $ciphertext_base64 = base64_encode($ciphertext);
    return rtrim($ciphertext_base64);
  }
/**
 * [AESDecrypt description]
 * @param [type] $ciphertext_base64 [description]
 * @param [type] $old_key           [description]
 */
  static function AESDecrypt($ciphertext_base64,$old_key){
    //$key = pack('H*', $old_key);
    $key = HEXDecode($old_key);
    # 为 CBC 模式创建随机的初始向量
    //$iv_size = mcrypt_get_iv_size(MCRYPT_RIJNDAEL_128, MCRYPT_MODE_ECB);
    ...
    //                                 $ciphertext_dec, MCRYPT_MODE_ECB, $iv_dec);
    //var_dump(HEXEncode($ciphertext_dec));
    $plaintext_dec = mcrypt_decrypt(MCRYPT_RIJNDAEL_128, $key,
  ...
  function HEXDecode($hex){
    ...
    for ($i=0; $i < strlen($hex)-1; $i+=2){
        ...
        //$tempString .= chr(((hexdec($hex[$i])&0xf)<<4)|(hexdec($hex[$i+1])&0xf));
    }
    //var_dump($string);
    //var_dump($tempString)
    return $string;
